...
auto_bin <- function(data) {
  ...
  return(c(
    0, ceiling(maxValue / 50), ceiling(maxValue / 25),
...
to_numeric_naToZero <- function(colname) {
  return(colname <- colname %>%
    apply(
      margin = 2,
      FUN = (if (is.na(as.numeric(colname))) {
        return(0)
      } else {
        return(as.numeric(colname))
      })
    ))
}
...
clean_textile_name <- function(data) {
  cleaned <- data %>%
    mutate(textile_name = str_replace_all(
      textile_name,
      "adaties",
      "adathaies"
    )) %>%
...
get_binByDataType <- function(data, dataType) {
  ...
  return(country.colors <- colorBin(
    palette = "YlOrRd",
    domain = col,
    bins = bins
  ))
